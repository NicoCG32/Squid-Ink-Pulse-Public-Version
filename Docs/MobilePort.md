...
Touch no se enlaza directamente a estas acciones: `UI/Point` y `UI/Click` son los únicos bindings touchscreen actuales. La superficie y los botones móviles posteriores alimentarán los comandos desde regiones explícitas para que un mismo gesto no pulse UI y active gameplay. El detalle y el estado de migración se mantienen en [Input.md](Input.md).
