...
Touch no se enlaza directamente a estas acciones: `UI/Point` y `UI/Click` son los únicos bindings touchscreen actuales. `TouchSteeringSurface` ya traduce una región explícita a posición vertical semántica con ownership de un solo dedo, prioridad sobre mouse y cancelación de lifecycle; los botones móviles se agregan después. El código de la superficie aún no está montado en prefabs ni escenas, por lo que una build Android todavía no ofrece movimiento touch jugable. El detalle y el estado de migración se mantienen en [Input.md](Input.md).
