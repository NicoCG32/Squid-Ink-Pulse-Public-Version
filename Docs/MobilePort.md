...
El mapa versionado `Gameplay` expresa `SteerPosition`, `ActivateInkPulse`, `TogglePause`, `UseGadgetSlot1` y `UseGadgetSlot2`. Keyboard&Mouse mantiene posición del mouse, click/`Space`, `P`/`Escape` y `Q`/`W`.

`SquidInkPulseGameplayInputReader` habilita exclusivamente ese mapa, publica posición continua y eventos discretos, y resuelve el esquema activo en un solo lugar. Su scope vive en el prefab del jugador y dispone el lector al abandonar gameplay. No acumula comandos: cada habilitación rechaza eventos anteriores al nuevo ciclo y limpia su estado transitorio al deshabilitarse. `PlayerMovement` consume `SteerPosition` e `InkPulseController` consume `InkPulseRequested`; los demás controladores se migran de forma separada para conservar una regresión acotada por sistema.
